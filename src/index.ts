...
class UsersData {
	...
	public add(user: User): void {
		if (typeof user.name === 'string' && typeof user.age === 'number' && user.age > 0 && user.name.trim().length > 0) {
			this.data.push(user);
		...
		.then(async (answers: InquirerAnswers) => {
			switch (answers.action) {
				...
				case Action.Edit:
					...
					users.edit(userToEdit.name, { name: userToEdit.newName, age: userToEdit.newAge });
					break;
				case Action.Remove:
